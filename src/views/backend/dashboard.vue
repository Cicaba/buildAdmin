        ...
        <el-col :md="24" :lg="18">
          <div class="welcome suspension">
            ...
            <div class="welcome-text">
              <div class="welcome-title">{{ adminInfo.nickname }}{{ state.greetings }}</div>
        ...
        <el-col :lg="6" class="hidden-md-and-down">
          <div class="working">
            ...
            <div class="working-text">
              您今天已工作了<span class="time">{{ state.workingTimeFormat }}</span>
            </div>
            <div @click="onChangeWorkState()" class="working-opt working-rest">
              {{ state.pauseWork ? "休息片刻" : "继续工作" }}
            </div>
          </div>
        </el-col>
      </el-row>
...
<script setup lang="ts">
  ...
  const initFileGrowthChart = () => {
      ...
      const option = {
          ...
          radar: {
              indicator: [{ name: '图片' }, { name: '文件' }, { name: '文档' }, { name: 'PDF' }],
          },
          series: (function () {
              var series = []
              for (var i = 1; i <= 12; i++) {
