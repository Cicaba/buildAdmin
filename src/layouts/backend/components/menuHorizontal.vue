<template>
  <div class="layouts-menu-horizontal">
    ...
    <el-scrollbar ref="horizontalMenusRef" class="horizontal-menus-scrollbar">
      <el-menu class="menu-horizontal" mode="horizontal" :default-active="state.defaultActive" :background-color="config.layout.menuBackground" :text-color="config.layout.menuColor" :active-text-color="config.layout.menuActiveColor" :key="state.menuKey">
          ...
          <template v-if="menu.children && menu.children.length > 0">
            <el-sub-menu :index="menu.path" :key="menu.path">
              <template #title>
                ...
                <span>{{ menu.title ? menu.title : "无标题" }}</span>
              </template>
